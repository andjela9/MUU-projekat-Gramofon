...
void wait2ms(void) {
	...
	TH0=0x0F8;				//isto kao prethodno samo brojis 2ms. da li znas kako se racunaju vrednosti koje se upisuju u th0 i tl0 ?
	TL0=0xCD;
	TR0=1;
	while(!TF0) {}
			TF0=0;
...
void initDisplay(void) {	 //ovo je inicijalizacija displeja na cetvorobitni rezim rada. 
	...
	LCD_EN=0;				   //posalju se gornja 4 bita naredbe, svaki put kad saljes 4 bita ide EN=1; pa posaljes; pa EN=0;
							   // ceka se _nop_() - to je funkcija za cekanje- i onda se salju donja 4 bita. i tako par puta za neke razlicite naredbe
	wait50micro();			   //pise ti pored koje su
...
void writeLine(unsigned char* message) { 	//funkcija koja pise jednu rec na displej- iterira kroz string i poziva funkciju writeChar za svako slovo
	unsigned char n=0;
	while(message[n]!='\0') {
		writeChar(message[n]);
		n++;
